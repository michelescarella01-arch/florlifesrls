      ...
      <main className="bg-background min-h-screen">
        ...
        <section className="pt-32 pb-16 md:pt-40 md:pb-20 px-6">
          <div className="max-w-7xl mx-auto">
            ...
            <p className="mt-8 text-lg md:text-xl text-muted-foreground max-w-2xl leading-relaxed">
              Scopri quali fiori sono disponibili durante l&apos;anno, le loro origini e i periodi migliori di raccolta.
            </p>
          </div>
        </section>
        ...
        <div className="sticky top-[73px] z-40 bg-background/95 backdrop-blur-sm border-b border-border">
          ...
          <div className="flex gap-6 md:gap-10 px-6 py-5 overflow-x-auto scrollbar-hide border-t border-border/30 justify-center">
            {[
              { key: "all", label: "Tutti" },
              { key: "italia", label: "Italia", color: "bg-emerald-600" },
              { key: "olanda", label: "Olanda", color: "bg-orange-500" },
              { key: "sudamerica", label: "Sud America", color: "bg-amber-600" },
              { key: "verde", label: "Verde", color: "bg-green-700" },
            ].map((filter) => (
              <button
                key={filter.key}
            ...
            <div className="flex items-baseline justify-between mb-12 pb-6 border-b border-border/50">
              <div>
                <h2 className="font-serif text-2xl md:text-3xl text-foreground">
                  {selectedMonth ? monthsFull[selectedMonth - 1] : "Tutto l'anno"}
                </h2>
                <p className="text-muted-foreground text-sm mt-1">
                  {sortedFlowers.length} varietà disponibili
                </p>
              </div>
            ...
            {sortedFlowers.length > 0 ? (
                ...
                {sortedFlowers.map((flower) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground md:text-right md:min-w-[140px]">
                      {flower.availability.map(a => a.period).join(" · ")}
                    </p>
                  </article>
                ))}
              </div>
              ...
              <div className="text-center py-24">
                <p className="text-muted-foreground text-lg">
                  Nessuna varietà trovata con i filtri selezionati.
                </p>
              </div>
            )}
          </div>
        ...
        <section className="py-20 px-6 bg-muted/30">
          <div className="max-w-3xl mx-auto text-center">
            <p className="text-muted-foreground text-sm leading-relaxed mb-8">
              La disponibilità indicata è orientativa e può variare in base a origine, qualità, condizioni climatiche e andamento dei mercati.
            </p>
            <Link
                ...
                <div className="space-y-2">
                  {selectedFlower.availability.map((avail) => (
                    <div key={avail.origin} className="flex items-center gap-3">
                      <span className={`w-2.5 h-2.5 rounded-full ${originColors[avail.origin]}`} />
                      <span className="text-foreground">{avail.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-foreground">
                  {selectedFlower.availability.length > 1 
                    ...
                    : selectedFlower.availability[0].period
                  }
                </p>
              </div>
            </div>
